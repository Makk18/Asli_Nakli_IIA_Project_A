...
## Suggested talking points for the demo day
1. **Concurrent Terminal Instances**: Show the 3 separate terminal windows running live HTTP database services on ports 5001, 5002, 5003, and 5004.
2. **Network Data Communication**: Look up an item in the GUI and show the terminal logs printing live incoming HTTP `POST /query` calls from the mediator.
3. **Innovative Decision Making**: Show Tab 2's flowchart and step-by-step trace explaining how automated provenance reasoning detects counterfeits without manual table joins.
4. **Live SQL Console**: Show Tab 3 running arbitrary queries across the cluster.
5. **Manual Data Entry**: Add a new medicine in Tab 4 and show it instantly appearing across the network.
